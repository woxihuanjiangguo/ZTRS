...
from navsim.planning.scenario_builder.navsim_scenario import ego_status_to_ego_state
...
class CacheOnlyDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.return_prev_es = return_prev_es
        if self.return_prev_es:
    ...
    def _load_scene_with_token(self, token: str) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
        ...
        if self.return_prev_es:
            ...
            frames = scene.frames
            scene_tokens = [f.token for f in frames]
            prev_idx = scene_tokens.index(token) - 1
            if prev_idx < 0:
                prev_idx = 0
            prev_frame = frames[prev_idx]
            prev_ego_status = prev_frame.ego_status
            prev_privileged_ego_state = ego_status_to_ego_state(
                prev_ego_status,
                vehicle_parameters=get_pacifica_parameters(),
                time_point=TimePoint(int(prev_frame.timestamp))
            )
            sim_states_root = os.getenv('NAVSIM_TRAJPDM_ROOT')
            sim_states = pickle.load(open(f'{sim_states_root}/sim_states_16384_navtrain/{token}/tmp.pkl', 'rb'))
            for k, v in sim_states.items():
                sim_states[k] = v.astype(np.float)
            targets.update(sim_states)
            return (features, targets, token, prev_privileged_ego_state)
...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.return_prev_es = is_training and return_prev_es
